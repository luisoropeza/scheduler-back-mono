...
A Spring Boot 4 REST API for a clinic appointment scheduler: patients book appointments into time slots (`Schedule`) that doctors (`Personal`) publish, staff confirm/cancel/reschedule them, and email notifications go out on state changes. Java 26 (toolchain), Gradle, PostgreSQL, JWT auth.
...
Patients and staff (`Personal`) are separate entity types/tables with separate auth endpoints (`/api/auth/patient/*` vs `/api/auth/personal/*`), each issuing the same kind of JWT. Controllers that need the caller's identity/role pull them out of the injected `Authentication` (id = `auth.getName()`, role = first authority stripped of `ROLE_` prefix — see `AppointmentController.extractRole`), then pass them into the service layer, which does its own authorization checks (e.g. a patient can only see their own appointments, a doctor can only see their own schedule) alongside `@PreAuthorize`. When adding endpoints with per-resource ownership checks, follow this same pattern rather than relying on `@PreAuthorize` alone.
...
**Notifications are event-driven**, not called inline from the service layer: `AppointmentServiceImpl` (or wherever an appointment state changes) publishes an `AppointmentEvent` (record: appointment, `ActionEvent`, actor role); `NotificationServiceImpl.handleEvent` is a `@TransactionalEventListener`, so it only fires after the transaction commits, and decides the notification recipient (patient vs doctor) based on the action and who performed it. Emails are built as inline HTML strings via `JavaMailSender`. When adding new appointment-affecting operations, publish an event rather than calling `NotificationService` directly.
